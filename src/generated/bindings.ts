...
async configImport(filePath: string) : Promise<Result<ConfigImportResult, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("config_import", { filePath }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
async appDataReset() : Promise<Result<boolean, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("app_data_reset") };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
async providerCopyApiKeyToClipboard(providerId: number) : Promise<Result<boolean, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("provider_copy_api_key_to_clipboard", { providerId }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
async skillLocalDelete(workspaceId: number, dirName: string) : Promise<Result<boolean, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("skill_local_delete", { workspaceId, dirName }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
export type UsageQueryParams = { period: string; startTs: number | null; endTs: number | null; cliKey: string | null; providerId: number | null }
